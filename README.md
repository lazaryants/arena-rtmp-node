...
Проект собран из проверенной конфигурации действующего сервера `video.curling76.ru`. Первый baseline намеренно сохраняет существующую схему `place1`–`place16` и передачу медиапотока без перекодирования.
...
Перед развёртыванием обязательно замените домен, пути сертификатов и все `CHANGE_ME_*` в конфигурации. Реальный `restream-config.json` нельзя добавлять в Git.
